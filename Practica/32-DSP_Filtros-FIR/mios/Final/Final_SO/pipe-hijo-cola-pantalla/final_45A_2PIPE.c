...
void mnjs1(int a){
	...
	//Escribe en el PIPE
	write(ipcPipeB[1], MENSAJE_1, sizeof(MENSAJE_1));
...
void mnjs2(int b){
	...
	//Escribe en el PIPE
	write(ipcPipeC[1], MENSAJE_2, sizeof(MENSAJE_2));
...
//***************************************************************************//		
	...
	if (pipe(ipcPipeB) == -1)
	{
		printf ("Error. No se crea Pipe B");
		exit(-1);
	}
	if (pipe(ipcPipeC) == -1)
	{
		printf ("Error. No se crea Pipe C");
		exit(-1);
	}
//***************************************************************************//		
//**************************** Cola de Mensajes *****************************//	 
...
//***************************************************************************//		
	...
	recievedChars = read(ipcPipeB[0], recieveBufferPipe, sizeof(recieveBufferPipe));
	...
	{//Si esta OK imprime el contenido de la tuberia y la manda a D por Cola de Mensajes
		...
		printf("Padre lee de la tuberia B: %s\n",recieveBufferPipe);
	...
	//Espera lectura de la tuberia
	recievedChars = read(ipcPipeC[0], recieveBufferPipe, sizeof(recieveBufferPipe));
	...
	{//Si esta OK imprime el contenido de la tuberia y la manda a D por Cola de Mensajes
		...
		printf("Padre lee de la tuberia C: %s\n",recieveBufferPipe);
